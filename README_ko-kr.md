...
3. **micah-sign-bot**에게 다이렉트 메세지로 `/sign-in`를 보냅니다.
...
5. 7일동안 반복하면 **Fans** 역할을 획득합니다. 만약 **Sponsor** 역할을 갖고 있다면, 3일이 소요됩니다.
...
4. 인젝터에 키가 필요하면 키를 복사하고 입력합니다.
5. 게임은 자동으로 시작됩니다. 인터페이스가 나타날 때까지 기다립니다.
...
3. **키 파일**을 다운로드하여 폴더에 너혹 `enc.json` 파일이 `v0.x.x.exe`와 같은 폴더에 있는지 확인하시오
4. `v0.x.x.exe`을 다시 실행합니다.
5. `GenshinImpact.exe` 또는 `YuanShen.exe` 선택 (`cfg.ini`를 확인하여 인젝터가 올바른 경로를 설정했는지 확인합니다.)
6. 게임은 자동으로 시작됩니다. 인터페이스가 나타날 때까지 기다립니다.
...
1. [릴리즈 페이지](https://github.com/Korepi/keyauth-cpp-library/releases)를 엽니다.
...
3. `GenshinImpact.exe` 또는 `YuanShen.exe` 선택 (`cfg.ini`를 확인하여 인젝터가 올바른 경로를 설정했는지 확인합니다.)
4. 게임은 자동으로 시작됩니다. 인터페이스가 나타날 때까지 기다립니다.
