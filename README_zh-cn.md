...
## ❓ 如何验证并登录 DISCORD + 获得fans身份组

1. 加入 [Cotton Buds Discord](https://discord.gg/korek)
2. 您将从 **帐户验证** 收到 DM，然后单击 **验证** 或单击 `#account-verify` 进行 **验证**。
...
3. 使用`/sign-in`直接向**micah-sign-bot**发送消息
...
5. 您必须这样做 7 天，然后您将获得 **Fans** 角色。 如果您有**sponsor**角色，只需要3天
...
1. 前往 [releases page](https://github.com/Cotton-Buds/colorpicker/releases)
2. 下载`F`版本 (ex. [F8](https://github.com/Cotton-Buds/colorpicker/releases/tag/F8))

### 使用方法

1. 运行 `injector.exe`. 
2. 选择 `Yuanshen.exe` 或者 `GenshinImpact.exe`(检查`cfg.ini`查看 injector 是否选择了正确的游戏路径) 
3. 转到 [我们的 Discord 频道](https://discord.gg/korek) `micah-bot-verify` 频道并输入 `/getkey` 来获取密钥。
...
4. 当injector询问时，将您获得的密钥复制injector中。
5. 游戏会自动启动，等待界面出现.
6. 按下 `TAB` 键以打开 [Korepi](https://github.com/Korepi/Korepi) GUI菜单.
...
1. 前往 [releases page](https://github.com/Cotton-Buds/calculator/releases)
2. 下载`v0.x.x` 版本 (ex. [v0.9.7](https://github.com/Cotton-Buds/calculator/releases/tag/v0.9.7))

### Usage

1. 运行 `v0.x.x.exe`.
2. 前往 [我们的 discord频道](https://discord.gg/korek) `micah-bot-verify`频道并输入`/verify`，然后粘贴**控制台**中显示的“HWID”以获取**密钥文件**.
...
4. 再次运行`v0.x.x.exe`.
5. 选择`GenshinImpact.exe`或`YuanShen.exe`. （检查`cfg.ini`以查看注入器是否选择了正确的游戏路径）
6. 游戏会自动启动，等待界面出现。
7. 按“TAB”打开 [Korepi](https://github.com/Korepi/Korepi) GUI菜单.
